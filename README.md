...
- `gemma` or `gemma4` -> `gemma4:26b` (available, but not in the default panel)
...
```bash
export OLLAMA_PANEL_MODELS=qwen2.5:7b,llama3
```
...
Structured JSON panel results now include transparent per-model outputs under `panel_judgments`, for example:
...
```json
{
  ...
  "panel_judgments": {
    "qwen2.5:7b": {"inside_circle": true, "confidence": 60, "explanation": "Simple."},
    "llama3": {"inside_circle": true, "confidence": 66, "explanation": "Understandable."}
  }
}
```
